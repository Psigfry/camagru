...
<div id="feed-buttons">
    ...
    if ($liked) {
        echo '<img src="assets/liked.png" id="feed-buttons-img"><span id="feed-buttons-txt"></span>';
    } else {
        echo '<img src="assets/like.png" id="feed-buttons-img"><span id="feed-buttons-txt"></span>';
    }
    echo '</button>
    <button id="feed-like-button" onclick="focusCommentInput(\'' . $post['id'] . '\');">
        <img src="assets/comment.png" id="feed-buttons-img"><span id="feed-buttons-txt">Комментарии</span>
    </button>
    <button id="feed-like-button" onclick="shareFacebook(\'' . $post['id'] . '\', \'' . $_SERVER['HTTP_HOST'] . '\')">
        <img src="assets/share.png" id="feed-buttons-img"><span id="feed-buttons-txt"></span>
    </button>
</div>
<hr id="feed-bar">
<div id="feed-likedby">';
    if ($likes_count <= 0) {
        ...
        echo '<p>Оценил : <span>' . $likes[0]['username'] . '</span></p>';
    } else if ($likes_count === 2) {
        echo '<p>Оценили : <span>' . $likes[0]['username'] . '</span>, <span>' . $likes[1]['username'] . '</span></p>';
    } else if ($likes_count === 3) {
        echo '<p>Оценили : <span>' . $likes[0]['username'] . '</span>, <span>' . $likes[1]['username'] . '</span><span>и другие</span></p>';
    } else {
        echo '<p>Оценили : <span>' . $likes[0]['username'] . '</span>, <span>' . $likes[1]['username'] . '</span><span>' . ($likes_count - 2) . ' и другие</span></p>';
    }
    echo '</div>
<div id="feed-legend">
...
<div id="feed-morecoms">';
    if ($comments_count <= 0) {
        ...
        echo '<p>1 комментарий</p>';
    } else {
        echo '<p>' . $comments_count . ' комментария</p>';
    }
    echo '</div>
...
<div id="feed-comment-field">
    <input required minlength="1" maxlength="140" id="field" type="text" name="new-comment" placeholder="Добавить комментарий">
    <button id="envoyer" onclick="commentPost(\'' . $post['id'] . '\');">Отправить</button>
</div>
</div>';
}

/*
    Загрузка дополнительних потостов SVG
*/
try {
...
if (count($posts) < $count['count']) {
    echo '<div id="load-more" title="Загрузить больше" onclick="loadMore();">
